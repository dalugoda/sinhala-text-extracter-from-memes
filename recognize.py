...
def predict_sentence(base_path, character_segments_path, image_name):
    ...
    for word, char_list in sorted(word_list.items()):
        ...
        for char in sorted(char_list):
            ...
            filter_min = 40
            filter_max = 2600
            black_pixel_count = np.sum(char_image == 0)

            if filter_min < black_pixel_count < filter_max:
    ...
    prob_file_write = codecs.open(base_path + 'step_12_0_probabilities/' + base_image_name_array[0] + ".txt", "w", "utf-8")
    prob_file_write.write(str(probabilities))
    prob_file_write.close()
